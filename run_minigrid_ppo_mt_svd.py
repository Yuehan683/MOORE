...
import moore.utils.networks_ppo_svd as Network
# visulization
...
MT_EXP = {"MT7": ["MiniGrid-DoorKey-6x6-v0", "MiniGrid-DistShift1-v0",
          ...
          "MT5": ["MiniGrid-DoorKey-6x6-v0", "MiniGrid-LavaGapS7-v0", "MiniGrid-RedBlueDoors-6x6-v0",
                  "MiniGrid-DistShift1-v0", "MiniGrid-MemoryS11-v0"]
          }
...
def run_experiment(args, save_dir, exp_id=0, seed=None):
    ...
    if args.wandb:
        wandb.init(name="seed_" + str(exp_id if seed is None else seed), project="MOORE",
                   group=f"minigrid_{args.env_name}", job_type=args.exp_name, entity=args.wandb_entity,
                   config=vars(args))
    ...
    if args.wandb:
        wandb.log({"all_minigrid/AverageReturn": current_all_average_return / n_contexts,
                   "all_minigrid/AverageDiscountedReturn": current_all_average_discounted_return / n_contexts}, step=0,
                  commit=True)
    ...
    for n in trange(n_epochs):
        ...
        for c, mdp_c in enumerate(mdp):
            ...
            agent.policy.set_beta(beta)
            core.current_idx = c
            dataset = core.evaluate(n_episodes=n_episodes_test, render=(
                args.render_eval if n % args.render_interval == 0 and exp_id == 0 else False))
            min_J, max_J, mean_J, mean_discounted_J, _ = get_stats(dataset, gamma, gamma_eval)
            metrics[mdp_c.env_name]["MinReturn"].append(min_J)
            metrics[mdp_c.env_name]["MaxReturn"].append(max_J)
            metrics[mdp_c.env_name]["AverageReturn"].append(mean_J)
